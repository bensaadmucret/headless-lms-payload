    ...
    try {
      ...
      for (const quiz of validQuizzes) {
        ...
        for (const partialQuestion of quizWithQuestions.questions) {
          ...
          if (partialQuestion.questionType !== 'multipleChoice') {
            continue;
          }
          ...
          try {
            ...
            if (questionRaw.options && Array.isArray(questionRaw.options)) {
              questionRaw.options = questionRaw.options.map((option: any) => ({
            ...
            if ((questionRaw as any).options && Array.isArray((questionRaw as any).options)) {
              question.options = (questionRaw as any).options.map((option: any) => ({
                id: option.id,
                text: option.text,
                isCorrect: option.isCorrect,
              }));
